...
class WhatsAppChannelAdapterSkeletonTest {
    @Test
    void returns_info() {
        var adapter = new WhatsAppChannelAdapterSkeleton();
        var out = adapter.handle(null);
        assertEquals(ClientOutputMessageType.INFO, out.type());
        assertEquals("whatsapp", out.payload().get("channel"));
    }
    ...
    void exposes_integration_ready_metadata_contract() {
        ...
        assertEquals(java.util.List.of("webhookToken", "accessToken", "webhookSecret", "phoneNumberId", "apiBaseUrl", "timeoutMs"), adapter.requiredConfigKeys());
    }
